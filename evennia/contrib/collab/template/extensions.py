...
class PronounsExtension(Extension):
    ...
    def parse(self, parser):
        ...
        # now we parse the body of the cache block up to `endpronouns` and
        # drop the needle (which would always be `endpronouns` in that case)
        body = parser.parse_statements(['name:endpro'], drop_needle=True)

        # now return a `CallBlock` node that calls our _pronoun_sub
        # helper method on this extension.
        block = nodes.CallBlock(self.call_method('pronoun_sub', args=[target]), [], [], body)
        block.set_lineno(lineno)
        return block

    @safe
    def pronoun_sub(self, target, caller):
